...
<!--HISTORY-->
<center><div align="center">
    <h1>Missav 배치 백업 컬렉션 비디오</h1>
    <p>「 현재 Missav 페이지에서 사진 파일 및 비디오 정보를 가져오고 결과를 병합 한 후 다운로드 된 웹 페이지 파일을 제공하십시오. 」</p>
    ...
    <img src="https://img.shields.io/greasyfork/dt/497682?logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&label=%EC%82%AC%EC%9A%A9%EC%9E%90%20%EC%88%98">
    <img src="https://img.shields.io/badge/dynamic/json?&label=%EB%A7%A4%EC%9D%BC%20%ED%99%95%EC%9D%B8%20%EC%97%85%EB%8D%B0%EC%9D%B4%ED%8A%B8&query=$.497682&logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&url=https://github.com/ChinaGodMan/UserScriptsHistory/raw/main/daily_update_checks.json">
...
Missav 배치 추출기는 즐겨 찾기를 백업하고 비디오 컨텐츠를 추출하는 도구입니다.。홈페이지를 제외한 모든 비디오를 크롤링 할 수 있습니다，또한 사용자 정의 추출 속도와 파일 이름 지정 기능을 제공합니다.。다운로드가 완료된 후，포함 된 그림을 생성하십시오 `img` 폴더와 하나 `파일 이름을 저장합니다.html` 이름 웹 파일，얻은 비디오 정보를 보는 데 사용됩니다。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 기능

-   **크롤링 비디오**：홈페이지를 제외한 모든 비디오를 크롤링 할 수 있습니다，비디오 재생 페이지가 테스트되지 않았습니다。
-   **버튼을 생성하십시오**：페이지의 오른쪽 상단에 버튼을 생성합니다.，추출 작업을 시작하는 데 사용됩니다。
-   **맞춤형 추출 속도**：원하는 속도로 비디오를 캡처 할 수 있습니다。
-   **다운로드 후 파일을 생성하십시오**：
    -   **img 그림 폴더**：비디오 썸네일을 저장하십시오。
    -   **jsonindex 웹 페이지 파일**：비디오 정보가 포함되어 있습니다，이미지 미리보기가 포함되어 있습니다，미리보기 비디오를 보려면 이미지를 클릭하십시오，제목을 클릭하여 점프하십시오 Missav 페이지 회전 및 표시 페이지를 사용자 정의하십시오。
-   **배치 다운로드**：배치 다운로드를 위해 모든 시트 병합을 지원합니다。
-   **배치 작동**：여러 필름 목록에서 비디오의 배치 추출을 지원합니다。
-   **사용자 정의 파일 이름**：사용자 정의 저장 파일 이름을 지원합니다。
-   **맞춤형 추출 속도**：비디오 추출 속도를 설정할 수 있습니다。
-   **HTML 생성하다인터페이스**：네트워크에서 사진을 얻고 생성하는 것을 지원합니다 HTML 인터페이스。
-   **고화질 큰 이미지를 다운로드하십시오**：고화질 큰 이미지의 다운로드 지원。
-   **지원하다WebDAV지원**：저장하십시오webdav。
...
-   **배치 다운로드 및 모든 시트를 병합합니다**：배치 다운로드를 위해 모든 시트 병합을 지원합니다。
-   **비디오 재생 인터페이스를 제외한 모든 비디오를 추출하십시오**：비디오 재생 인터페이스를 제외한 모든 비디오를 추출하십시오。
-   **배치 작업을 위해 배치에서 시트를 선택하십시오**：작동 용 시트의 배치 선택을 지원합니다。
-   **팝업 입력 상자를 제거하십시오**：불필요한 입력 상자를 제거했습니다。
-   **사용자 정의 다운로드 파일 이름 저장**：사용자 정의 다운로드 파일 이름을 지원합니다。
-   **맞춤형 추출 속도**：추출 속도를 설정할 수 있습니다。
-   **HTML 생성인터페이스는 네트워크에서 사진을 얻는 것을 지원합니다**：생성 HTML 인터페이스는 네트워크에서 사진을 얻을 수 있습니다。
-   **고화질 큰 이미지를 다운로드하십시오**：고화질 큰 이미지의 다운로드 지원。
...
-   **마지막 페이지를 검색 한 후 최대 페이지를 초과하는 정보를 얻을 수 있습니다.**：나는 실제로 그것을 얻지 못할 것입니다。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 사용 지침

1. **비디오 분류에 따라 홈페이지에서 사용하십시오**：비디오 분류 페이지의 홈페이지를 엽니 다。
2. **오른쪽 상단의 버튼을 클릭하십시오**：추출 작업을 시작하십시오。
3. **추출 속도를 설정하십시오**：필요에 따라 추출 속도를 조정하십시오。
4. **비디오와 사진을 다운로드하십시오**：추출이 완료되면 파일과 사진이 생성됩니다.。
5. **확인하다 `jsonindex` 웹 페이지 파일**：비디오 정보를 봅니다，미리보기를 보려면 이미지를 클릭하십시오，제목을 클릭하여 점프하십시오 Missav 페이지。
