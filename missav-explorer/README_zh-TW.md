...
<!--HISTORY-->
<center><div align="center">
    <h1>MissAv批量備份收藏視頻</h1>
    <p>「 從當前missav頁面獲取圖片文件和視頻信息，並合併結果後提供下載生成的網頁文件 」</p>
    ...
    <img src="https://img.shields.io/greasyfork/dt/497682?logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&label=%E7%94%A8%E6%88%B6%E6%95%B8%E9%87%8F">
    <img src="https://img.shields.io/badge/dynamic/json?&label=%E6%AF%8F%E6%97%A5%E6%AA%A2%E6%9F%A5%E6%9B%B4%E6%96%B0%E6%AC%A1%E6%95%B8&query=$.497682&logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&url=https://github.com/ChinaGodMan/UserScriptsHistory/raw/main/daily_update_checks.json">
    <p>Download:<a href="https://github.com/ChinaGodMan/UserScripts/tree/main/missav-explorer">Github</a> | ⭐<a
            ...
            Fork</a></p><details><summary style="color:#FFD700">更新記錄</summary><div align="right"><a href="https://github.com/ChinaGodMan/UserScripts/tree/main/missav-explorer/CHANGELOG.md"><img src="https://img.shields.io/badge/-GitHub-3A3A3A?style=flat&logo=GitHub&logoColor=white"></a></div><h1><strong>🛠️ MissAV 批量備份收藏視頻 更新日誌</strong></h1>
...
Missav 批量提取器是一個用於備份收藏夾和提取視頻內容的工具。它可以抓取除了主頁下的所有視頻，並提供了自定義提取速度和文件命名的功能。下載完成後，生成一個包含圖片的 `img` 文件夾和一個 `保存文件名.html` 命名網頁文件，用於查看獲得的視頻信息。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 功能

-   **抓取視頻**：可以抓取除主頁外的所有視頻，視頻播放頁面未測試過。
-   **生成按鈕**：在頁面右上角生成一個按鈕，用於啟動提取操作。
-   **自定義提取速度**：可以按自己想要的速度進行視頻抓取。
-   **下載完成後生成文件**：
    -   **img 圖片文件夾**：存儲視頻縮略圖。
    -   **jsonindex 網頁文件**：包含視頻信息，包括圖片預覽，點擊圖片可查看預覽視頻，點擊標題可跳轉到 Missav 自定義翻頁和顯示的頁面。
-   **批量下載**：支持合併所有片單進行批量下載。
-   **批量操作**：支持從多個片單中批量提取視頻。
-   **自定義文件名稱**：支持自定義保存文件的名稱。
-   **自定義提取速度**：可以設置提取視頻的速度。
-   **HTML 生成介面**：支持從網絡獲取圖片並生成 HTML 介面。
-   **高清大圖下載**：支持下載高清大圖。
-   **支持WebDAV備份**：在下載同時保存到webdav。
...
-   **批量下載合併所有片單**：支持將所有片單合併進行批量下載。
-   **除視頻播放界面的所有視頻提取**：提取除了視頻播放界面外的所有視頻。
-   **批量選取片單進行批量操作**：支持批量選取片單進行操作。
-   **去除彈出輸入框**：去掉了不必要的輸入框。
-   **自定義下載保存文件名稱**：支持自定義下載文件的名稱。
-   **自定義提取速度**：可以設置提取速度。
-   **HTML 生成的界面支持從網絡獲取圖片**：生成的 HTML 界面可以從網絡獲取圖片。
-   **下載高清大圖**：支持下載高清大圖。
...
-   **最後一頁獲取完成後提示獲取超出最大頁的信息**：實際不會去獲取。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 使用說明

1. **在視頻分類下的首頁使用**：打開視頻分類頁面的首頁。
2. **點擊右上角的按鈕**：啟動提取操作。
3. **設置提取速度**：根據需要調整提取速度。
4. **下載視頻和圖片**：提取完成後會生成文件和圖片。
5. **查看 `jsonindex` 網頁文件**：查看視頻信息，點擊圖片查看預覽，點擊標題跳轉到 Missav 頁面。
