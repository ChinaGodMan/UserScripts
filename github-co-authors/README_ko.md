...
1. **첫 번째 실행 초기화**  
   스크립트가 처음 실행될 때，사용자가 입력하라는 메시지가 표시됩니다 GitHub ~의 Token。Token 안전하게 저장됩니다，후속 통화에 사용됩니다 GitHub API。

2. **동적 페이지로드 지원**  
   대본이들을 것입니다 GitHub 페이지의 탐색 이벤트（`turbo:render`），페이지가 토글 할 때 버튼을 사용할 수 있는지 확인하려면。

3. **사용자 인터페이스**  
   존재하다 PR 페이지에 버튼을 추가하십시오 "Add co-authors" 상태 프롬프트 상자，사용자는 한 번의 클릭 으로이 버튼을 추가 할 수 있습니다 co-author 정보。

4. **참가자를 자동으로 취득합니다**
    - 스크립트는 현재의 스크립트입니다 PR 다음 출처에서 참가자를 추출하십시오：
        - PR 논평
        - Review 논평
        - 감사 기록
    - 각 참가자에 대해 생성하십시오 `Co-authored-by` 형식 메시지。

### 용법

1. 설치 스크립트
2. 열려 있는 GitHub PR 페이지。
3. 스크립트를 클릭하여 자동으로 생성하십시오 "Add co-authors" 단추。
4. 스크립트는 참가자의 자동으로 전송합니다. `Co-authored-by` 커밋 메시지에 정보를 추가하십시오。
