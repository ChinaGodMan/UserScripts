...
1. **Đầu tiên chạy khởi tạo**  
   Khi tập lệnh được chạy đầu tiên，Người dùng sẽ được nhắc nhập GitHub của Token。Token Sẽ được lưu trữ an toàn，Được sử dụng cho các cuộc gọi tiếp theo GitHub API。

2. **Hỗ trợ tải trang động**  
   Kịch bản sẽ lắng nghe GitHub Các sự kiện điều hướng trên trang（`turbo:render`），Để đảm bảo rằng nút vẫn có sẵn khi trang chuyển đổi。

3. **Giao diện người dùng**  
   hiện hữu PR Thêm một nút vào trang "Add co-authors" và hộp nhắc trạng thái，Người dùng có thể thêm nút này chỉ bằng một cú nhấp chuột co-author thông tin。

4. **Tự động có được người tham gia**
    - Kịch bản sẽ là từ hình ảnh hiện tại PR Trích xuất người tham gia từ các nguồn sau đây：
        - PR Bình luận
        - Review Bình luận
        - Hồ sơ kiểm toán
    - Tạo cho mỗi người tham gia `Co-authored-by` Thông báo định dạng。

### cách sử dụng

1. Tập lệnh cài đặt
2. Mở GitHub PR trang。
3. Nhấp vào tập lệnh để tạo tự động "Add co-authors" Cái nút。
4. Kịch bản sẽ tự động chuyển người tham gia ’ `Co-authored-by` Thêm thông tin vào tin nhắn cam kết。
