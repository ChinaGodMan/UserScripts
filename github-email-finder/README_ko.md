...
<!--HISTORY-->
<center><div align="center">
    <h1>GitHub 이메일 주소 찾기</h1>
    <p>「 공개 이벤트 데이터 (예 : 공개 코드 기반으로 푸시 된 제출)를 사용하고 사용자의 제출 이메일 주소를 가져 와서 프로필 페이지에 표시하십시오. 또한 쉽게 복사 할 수 있도록 편리한 "Copy to Clipboard"버튼을 추가합니다. 」</p>
    ...
    <img src="https://img.shields.io/greasyfork/dt/537309?logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&label=%EC%82%AC%EC%9A%A9%EC%9E%90%20%EC%88%98">
    <img src="https://img.shields.io/badge/dynamic/json?&label=%EB%A7%A4%EC%9D%BC%20%ED%99%95%EC%9D%B8%20%EC%97%85%EB%8D%B0%EC%9D%B4%ED%8A%B8&query=$.537309&logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&url=https://github.com/ChinaGodMan/UserScriptsHistory/raw/main/daily_update_checks.json">
...
이 스크립트는 사용됩니다 GitHub 사용자 홈페이지는 사용자 공개 제출물을 자동으로 추출합니다（PushEvent）이메일 주소(공개 이메일 주소가없는 경우,)，이메일 정보를 페이지에 삽입하십시오，또한 한 번의 클릭으로 클립 보드에 이메일을 복사하는 기능도 제공합니다.。

## 기능적 개요

-   현재 페이지에 이메일 정보가 표시되는지 확인。
-   그렇지 않다면，~에서 GitHub API 사용자 공개 이벤트 데이터를 얻습니다，가장 최근에 추출하십시오 PushEvent 이메일을 제출하십시오。
-   추출 된 이메일을 사용자 프로필 페이지의 연락처 정보 영역에 삽입하십시오.。
-   사서함 옆에 클립 보드 아이콘을 삽입하십시오，이메일을 클립 보드에 복사하려면 클릭하십시오。
-   성공적인 복사 후，클립 보드 아이콘이 진드기 아이콘으로 대체됩니다.，성공적으로 즉시 복사하십시오。

## 사용 지침

1. 구성이 유효한지 확인하십시오 GitHub 개인 액세스 토큰（`GITHUB_TOKEN`）[여기](https://github.com/settings/tokens/new?description=Github%20Email%20Finder&scopes=repo)새로운 토큰을 생성하십시오，피하기 위해 API 요청 제한。

## 알아채다

-   이 스크립트는 정확한 이메일 주소를 얻지 못합니다，제출자의 이메일 주소입니다，아마도 사용자의 등록 이메일이 아닐 것입니다。(예를 들어, Merge 요청의 이메일 주소입니다,그런 다음 표시되는 이메일 주소는 Merge 요청의 개시 자의 이메일 주소입니다.)
-   이 스크립트는 현재 테스트 중입니다，몇 가지 문제가 있습니다。
