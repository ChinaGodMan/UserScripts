 ...
 * -----
 * Last Modified: 2026/06/06,Saturday 17:04:45
...
const addSizeToRepos = () => {
    ...
    document.querySelectorAll(repoSelector).forEach(async (elem) => {
        ...
        if (pageType === 'repo') {
            // const reposApi = isLoggedInUser(jsn.owner.login)
            const reposApi = isLoggedInUser_f()
...
function isLoggedInUser(avatar_url) {//从返回的json判断
    ...
    return avatar_url.includes(username)
}
