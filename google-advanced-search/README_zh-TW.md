...
<center><div align="center">
    ...
    <img src="https://img.shields.io/greasyfork/dt/502652?logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&label=%E7%94%A8%E6%88%B6%E6%95%B8%E9%87%8F">
    <img src="https://img.shields.io/badge/dynamic/json?&label=%E6%AF%8F%E6%97%A5%E6%AA%A2%E6%9F%A5%E6%9B%B4%E6%96%B0%E6%AC%A1%E6%95%B8&query=$.502652&logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&url=https://github.com/ChinaGodMan/UserScriptsHistory/raw/main/daily_update_checks.json">
    <p>Download:<a href="https://github.com/ChinaGodMan/UserScripts/tree/main/google-advanced-search">Github</a> | ⭐<a
            ...
            Fork</a></p><details><summary style="color:#FFD700">更新記錄</summary><div align="right"><a href="https://github.com/ChinaGodMan/UserScripts/tree/main/google-advanced-search/CHANGELOG.md"><img src="https://img.shields.io/badge/-GitHub-3A3A3A?style=flat&logo=GitHub&logoColor=white"></a></div><h1><strong>🛠️ Google 高級搜索 更新日誌</strong></h1>
<h3><strong>📅 2024/8/5 23:18 - Ver 0.1.9.1</strong></h3>
<p><strong>新增</strong>: 增加腳本圖標</p>
<hr />
<h3><strong>📅 2024/8/5 10:26 - Ver 0.1.9</strong></h3>
<p><strong>issues</strong>: 移動設備無法顯示高級搜索框。<br />
<strong>修復</strong>: 下面代碼僅在 PC 端生效 <code>searchContainer.appendChild(toggleButton)</code>。</p>
<hr /></details>
...
這是一個為 Google 添加高級搜索表單的用戶腳本。它能在頁面頂部添加一個可隱藏的高級搜索表單，使您能夠更精確地搜索信息。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 功能

-   在 Google 搜索頁面頂部添加一個 “高級搜索” 按鈕，點擊按鈕可顯示高級搜索表單。
-   高級搜索表單包含了以下搜索選項：
    -   `以下所有字詞`：搜索結果中必須包含所有指定的關鍵字。
    -   `與以下字詞完全匹配`：搜索結果中必須包含完全匹配指定字詞的結果。
    -   `以下任意字詞`：搜索結果中必須包含指定的任意一個關鍵字。
    -   `排除以下字詞`：搜索結果中不包含指定的關鍵字。
    -   `包含的數字範圍`：搜索結果中包含指定範圍內的數字。
    -   `最後更新時間`：搜索結果中包含指定更新時間的結果。
    -   `網站或域名`：搜索結果中包含指定網站或域名的結果。
    -   `文件類型`：搜索結果中包含指定文件類型的結果。
-   可以保存以前的搜索選項，這樣在打開新頁面時會自動填充表單。
-   提供清空按鈕，可以清除表單中的數據。

## 感謝

腳本修改自 **[shiquda](https://greasyfork.org/users/935206)** 發布的腳本 [Google Advanced Search Assistant](https://greasyfork.org/scripts/474178)
