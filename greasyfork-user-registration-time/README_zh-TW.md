...
<!--HISTORY-->
<center><div align="center">
    <h1>顯示Greasyfork用戶註冊時間，識別惡意評論</h1>
    ...
    <p>Download:<a href="https://github.com/ChinaGodMan/UserScripts/tree/main/greasyfork-user-registration-time">Github</a> | ⭐<a
            ...
            Fork</a></p><details><summary style="color:#FFD700">更新記錄</summary><div align="right"><a href="https://github.com/ChinaGodMan/UserScripts/tree/main/greasyfork-user-registration-time/CHANGELOG.md"><img src="https://img.shields.io/badge/-GitHub-3A3A3A?style=flat&logo=GitHub&logoColor=white"></a></div><h1><strong>🛠️ greasyfork-user-registration-time 更新日誌</strong></h1>
<h3><strong>📅 22025.04.12.1109</strong></h3>
<p><strong>修復</strong>: 添加<code>fr</code> 和 <code>fr-CA</code>語言的支持,以顯示法國時間格式<code>10.04.2025 20:38</code><a href="https://greasyfork.org/scripts/529359/discussions/295260">#295260</a> <br>
<strong>截圖</strong>:<img alt="2025.04.12" src="https://s2.loli.net/2025/04/12/EqfUv7T6l9ebcLB.png" /></p>
<h3><strong>📅 2025.03.10.0959</strong></h3>
<p><strong>新增</strong>: 從<code>GreasyFork</code>直接獲取用戶的註冊時間 ..</p>
<hr /></details>
...
# 顯示 Greasyfork 用戶註冊時間，識別惡意評論

## 功能介紹

該腳本用於在 GreasyFork 或 SleazyFork 上顯示用戶的**註冊時間**，幫助識別**惡意註冊**的賬戶。惡意註冊的賬戶通常會在**註冊後不久**給予差評，且**賬號活動較少**。通過顯示用戶的註冊時間，可以推測用戶的可信度。如果註冊時間顯示為**未來的時間**，則該用戶很可能是**惡意註冊**的。
這個腳本在 greasyfork 的各種評論、留言相關頁面顯示發言者的註冊時間。如果你是開發者，可以幫你分辨惡意評論的小號；如果你不是開發者，也可以幫你辨認發言者是老用戶還是新用戶。

## 功能亮點：

-   **顯示用戶註冊時間**：腳本會自動獲取用戶的註冊時間，並將其顯示在用戶名旁邊，幫助區分新用戶與長期活躍用戶。
-   **推測註冊時間**：通過訪問 GreasyFork 提供的 API，腳本推測並顯示每個用戶的註冊時間。
-   **識別惡意賬戶**：註冊時間若為未來時間，表示該用戶可能是惡意註冊的賬號，警示用戶注意。
...
1. 💯 安裝該腳本後，它會在你瀏覽 GreasyFork 或 SleazyFork 網站時自動加載。
2. 👍 當頁面加載完成後，腳本會在用戶信息旁邊顯示該用戶的註冊時間。
3. ⏲️ 如果頁面上已有該用戶的註冊時間，腳本會直接顯示；如果沒有，會顯示一個按鈕，點擊後可以獲取並顯示該時間。(腳本直接通過`GreasyFork`提供的 API 獲取註冊時間，為了防止被屏蔽,所以添加按鈕手動獲取)
4. 🇹🇱 默認顯示的時間格式為`YYYY-MM-DD HH:mm:ss`。腳本變量`absoluteTime`改為`relative-time`使用網站的時間顯示.

## 適用環境：

-   💯 支持 Chrome、Firefox、Edge、Opera、Safari、Kiwi 等瀏覽器。

## 聯繫方式：

-   👨‍🦰 **作者**: 人民的勤务员@ChinaGodMan
-   📧 **郵箱**: china.qinwuyuan@gmail.com
-   🕸️ **項目主頁**: [UserScripts](https://github.com/ChinaGodMan/UserScripts)

<!--AUTHORS-->
## 💖 腳本參考或使用了以下腳本:
[@yysk.org](https://greasyfork.org/users/922552) ---- [顯示Greasyfork用戶註冊時間，識別惡意評論](https://greasyfork.org/scripts/450603)
<!--AUTHORS-END-->
<!--SCREENSHOT-->

## 截圖預覽:

1. _匹配評論列表頁面：[https://greasyfork.org/zh-CN/scripts/\\d+/feedback](https://greasyfork.org/zh-CN/scripts/\d+/feedback)_

![](https://s3.bmp.ovh/imgs/2022/09/02/42c32b26a20c4b01.png)

2. _匹配評論頁面：[https://greasyfork.org/zh-CN/scripts/\\d+/discussions/\\d](https://greasyfork.org/zh-CN/scripts/\d+/discussions/\d)+_

3. ![](https://s3.bmp.ovh/imgs/2022/09/02/9d5924d4f8412416.png)

4. _匹配用戶討論頁面：[https://greasyfork.org/zh-CN/discussions?user=\\d](https://greasyfork.org/zh-CN/discussions?user=\d)+_

![](https://s3.bmp.ovh/imgs/2022/09/02/993a3ac7ec9fd2ab.png)

5._匹配討論列表頁：[https://greasyfork.org/zh-CN/discussions](https://greasyfork.org/zh-CN/discussions)_
...
7. _匹配用戶列表頁 [https://greasyfork.org/zh-CN/users](https://greasyfork.org/zh-CN/users)_

![](https://s3.bmp.ovh/imgs/2022/09/03/aa2e429f08ddc797.png)

8. _匹配腳本表頁：[https://greasyfork.org/zh-CN/scripts](https://greasyfork.org/zh-CN/discscriptsussions)_
