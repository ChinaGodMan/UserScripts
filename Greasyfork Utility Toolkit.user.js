...
if(userlocalfav){
    ...
    addNavLink('书签', "https://greasyfork.org/Bookmarks",true );
...
function setStar(li, condition) {
    ...
    // 功能-脚本和主页名称清理 https://greasyfork.org/zh-CN/scripts/431940
    const m = /(\/[^/]+\/(scripts|users)\/\d+)-[^/]+(\/.*)?/.exec(location.pathname);
    if (m && cleanscriptname) {
        history.replaceState({}, null, `${location.origin}${m[1]}${m[3] ?? ''}${location.search}${location.hash}`);
    }
      ...
      <section>
        <header>
          <h3>${scr.name}: ${translate('dallScripts')}
            <button id="close-button" style="margin-left: 10px;" onclick="restoreBackup()">${translate('Close')}</button>
          </h3>
        </header>
        <section class="text-content">
          <ul>
            <span style="font-size: 15px;border-radius: 3px;background: rgb(45, 45, 45);color: rgb(255, 255, 255);margin: 0px 4px;padding: 0 4px;outline: 2px solid ${color};">
              <span style="width: 8px;height: 8px;display: inline-flex;margin-bottom: 1px;margin-left: 4px;border-radius: 50%;background: ${color};"></span> ${translate('JSScripts')} ${jsCount.toLocaleString()}
            </span><br>
            <span style="font-size: 15px;border-radius: 3px;background: rgb(45, 45, 45);color: rgb(255, 255, 255);margin: 0px 4px;padding: 0 4px;outline: 2px solid ${color};">
              <span style="width: 8px;height: 8px;display: inline-flex;margin-bottom: 1px;margin-left: 4px;border-radius: 50%;background: ${color};"></span> ${translate('CSSScripts')} ${cssCount.toLocaleString()}
            </span><br>
            <span style="font-size: 15px;border-radius: 3px;background: rgb(45, 45, 45);color: rgb(255, 255, 255);margin: 0px 4px;padding: 0 4px;outline: 2px solid ${color};">
              <span style="width: 8px;height: 8px;display: inline-flex;margin-bottom: 1px;margin-left: 4px;border-radius: 50%;background: ${color};"></span> ${translate('dallScripts')} ${scriptData.length}
            </span><br>
            <span style="font-size: 15px;border-radius: 3px;background: rgb(45, 45, 45);color: rgb(255, 255, 255);margin: 0px 4px;padding: 0 4px;outline: 2px solid ${color};">
            ...
            <span style="font-size: 15px;border-radius: 3px;background: rgb(45, 45, 45);color: rgb(255, 255, 255);margin: 0px 4px;padding: 0 4px;outline: 2px solid ${color};">
              <span style="width: 8px;height: 8px;display: inline-flex;margin-bottom: 1px;margin-left: 4px;border-radius: 50%;background: ${color};"></span> ${translate('TotalInstalls')} ${TotalInstalls.toLocaleString()}
            </span><br>
            <span style="font-size: 15px;border-radius: 3px;background: rgb(45, 45, 45);color: rgb(255, 255, 255);margin: 0px 4px;padding: 0 4px;outline: 2px solid ${color};">
              <span style="width: 8px;height: 8px;display: inline-flex;margin-bottom: 1px;margin-left: 4px;border-radius: 50%;background: ${color};"></span> ${translate('ok')} ${ok.toLocaleString()}
            </span><br>
            <span style="font-size: 15px;border-radius: 3px;background: rgb(45, 45, 45);color: rgb(255, 255, 255);margin: 0px 4px;padding: 0 4px;outline: 2px solid ${color};">
              <span style="width: 8px;height: 8px;display: inline-flex;margin-bottom: 1px;margin-left: 4px;border-radius: 50%;background: ${color};"></span> ${translate('bad')} ${bad.toLocaleString()}
            </span><br>
            <span style="font-size: 15px;border-radius: 3px;background: rgb(45, 45, 45);color: rgb(255, 255, 255);margin: 0px 4px;padding: 0 4px;outline: 2px solid ${color};">
    ...
    if (ulElement && usersearchingreasyfork) {
        ...
        if (showlinktotal) {
            links.forEach(link => {
                ...
                if (titleText) {
                    const numberMatch = titleText.match(/\d+/); // 使用正则表达式匹配数字
                    if (numberMatch) {
                        // 创建一个包含数字和音符的 span 元素
                        const span = document.createElement('span');
                        span.className = 'title-number';
                        span.style.fontSize = '12px'; // 调整为较小的字体大小
                        span.style.color = '#007bff'; // 调整字体颜色
        ...
        // 添加点击事件监听器
        $(ulElement).on('click', 'a, span', function(event) {
            event.preventDefault();
...
function applyDiscussionReadStyles(hide, applyStyles) {
    ...
    if (applyStyles) {
      css += 'color: gray;font-size: 0.9em;font-style: italic;';
    }
    css += '}';
