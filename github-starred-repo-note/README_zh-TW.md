...
這是一個可以給你 star 過的倉庫添加筆記的的腳本,並且你備註的信息可以上傳下載到 GitHub Gist

## 簡介

曾經 Star 過的倉庫是不是忘記了它們的用途？**GitHub RepoNotesHelper** 是一個為你量身定制的腳本，允許你為 Star 的倉庫添加備註。更棒的是，你的備註可以上傳到 **GitHub Gist**，隨時備份和恢復，方便又安全！

---

## 功能

-   **添加備註**：為 Star 的 GitHub 倉庫添加自定義備註，方便記憶。
-   **輕鬆管理**：備註直接嵌入到倉庫頁面，直觀且高效。
-   **數據同步**：支持將備註上傳到 GitHub Gist，隨時下載恢復數據。
-   **無縫集成**：自動檢測頁面上的所有倉庫並添加備註功能，無需手動干預。
