...
<center><div align="center">
    <h1>웹 페이지 작성 클립 버전 승인 관리자</h1>
    <p>「 무단 자동 복사 및 비활성화 및 비활성화하여 오디오 및 비디오 컨텐츠의 사전로드 및 자동 재생을 방지하기 위해 클립 보드에 웹 페이지 작성 동작 관리。 」</p>
    ...
    <p>Download:<a href="https://github.com/ChinaGodMan/UserScripts/tree/main/clipboard-manager">Github</a> | ⭐<a
            ...
            Fork</a></p><details><summary>레코드 업데이트</summary><h1><strong>🛠️ 웹 페이지 작성 클립 버전 승인 관리자 로그 업데이트</strong></h1>
...
스크립트는 다른 곳에서 다운로드되었습니다，저자가 누구인지 잘 모르겠습니다，건선을 예방하십시오。사용 ChatGPT 수정，스크립트가되게하십시오 PC 바로 가기 키를 사용하여 측면을 탐색 할 때 복사 할 수 있습니다.，웹 페이지 배경이 다양한 비밀번호를 복사하는 것을 특히 방지하십시오。이 스크립트는 복제 제한과 호환되지 않을 수 있습니다.！
스크립트가 적용되는 경우，반투명 작은 점은 웹 페이지의 오른쪽 하단에 표시됩니다.，다른 색상은 다른 상태에 해당합니다，설명은 다음과 같습니다：

-   **빨간색** : 모든 복제 동작을 가로 채십시오，승인 여부를 알리는 팝업 창。
-   **녹색** : 모든 복제 동작이 출시됩니다，그것은 스크립트가 실행되지 않음을 의미합니다。
-   **오렌지색** : 임시 차단，이 상태는 수동으로 복제 할 때만 발생합니다，복사 후 사용자가 자동으로 빨간색으로 변합니다，가로 채기를 복구하십시오。
-   **빨간색 점은 기본적으로 표시되지 않습니다**，클립 보드 쓰기 동작이 감지 된 후에 표시됩니다. 4s。현재 상태를 클릭하여 상태를 전환 할 수 있습니다，이 웹 페이지의 클립 보드 작성 동작을 해제할지 여부를 선택하십시오.。
...
이 프로젝트는 채택됩니다 MIT 특허。

## 질문 피드백

궁금한 점이 있으면，제발 통과하십시오 [GitHub Issues](https://github.com/ChinaGodMan/UserScripts/issues) 피드백을 제출하십시오。
