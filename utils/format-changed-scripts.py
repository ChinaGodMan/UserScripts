...
for file in changed_files:
    if file.endswith('.js'):
        ...
        if os.path.exists(file_path):
            ...
            matching_script = next((script for script in scripts_data['scripts'] if script['path'] == file_name), None)
            if not matching_script:
                print(f'文件 {file_name}不在列表中,无需格式化')
                continue
            if not matching_script.get('GreasyFork') or not matching_script['GreasyFork'].strip():
