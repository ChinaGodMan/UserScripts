...
<center><div align="center">
    <h1>小窗預覽</h1>
    <p>「 拖曳連結時在彈出視窗中開啟鏈接，並在打開前提供預覽，使用 Edge 的預讀技術。同時在小窗口打開時在背後添加壓克力效果。 」</p>
    ...
    <img src="https://img.shields.io/greasyfork/dt/504880?logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&label=%E7%94%A8%E6%88%B6%E6%95%B8%E9%87%8F">
    <img src="https://img.shields.io/badge/dynamic/json?&label=%E6%AF%8F%E6%97%A5%E6%AA%A2%E6%9F%A5%E6%9B%B4%E6%96%B0%E6%AC%A1%E6%95%B8&query=$.504880&logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&url=https://github.com/ChinaGodMan/UserScriptsHistory/raw/main/daily_update_checks.json">
...
<strong>修復</strong>: 修復彈出<code>sweetalert</code>沒有 css 的導致界面錯亂. <br></p>
<h3><strong>📅 2024/9/30 11:56 - Ver 2.5.1.4</strong></h3>
<p><strong>最佳化</strong>: 添加<code>GUI設置界面</code> 菜單太麻煩,設置多個點來點去的.</p>
<hr />
<h3><strong>📅 2024/9/30 10:49 - Ver 2.5.1.3</strong></h3>
<p><strong>最佳化</strong>: 添加<code>長按生效時間</code>,鼠標按下等於設置的生效時間後調用打開小窗前的<code>長按觸發時間</code>.<br>打開小窗時間=<code>長按生效時間</code>+<code>長按觸發時間</code>
<strong>最佳化</strong>: 添加<code>GUI設置界面</code> 菜單太麻煩,設置多個點來點去的.</p>
<hr />
<h3><strong>📅 2024/9/10 11:05 - Ver 2.5.1.0</strong></h3>
<p><strong>修復</strong>: 無法跨源讀取彈出窗口大小位置的問題。
<strong>新增</strong>: 當無法跨源讀取到彈窗位置時，會發送信息給彈窗，允許彈窗自己設置大小和位置。</p>
<hr />
<h3><strong>📅 2024/9/10 09:24 - Ver 2.5.0.0</strong></h3>
<p><strong>感謝</strong>: <a href="https://greasyfork.org/zh-CN/users/20361">tony0809</a> 的代碼貢獻。
<strong>新增</strong>:
• 開啟 <code>記錄窗口位置</code> 時，自動記錄當前域名配置。
• 查找窗口配置邏輯：域名配置 → 自定義窗口配置 → 腳本自帶配置。
• 配置無需手動編輯,腳本會自動在你更改窗口大小時更新域名配置.每次你改變的窗口大小會記錄到全局自定義配置
• 配置規則參考<a href="https://greasyfork.org/zh-CN/scripts/504880-small-window-preview/discussions/258907#comment-529654">#529654</a>
• 新增功能演示請參考下方視頻。</p>
<hr />
<h3><strong>📅 2024/9/3 05:26 - Ver 2.4.0.20</strong></h3>
<p><strong>修復</strong>: <a href="https://greasyfork.org/zh-CN/scripts/504880-small-window-preview/discussions/258301#comment-527837">#527837</a> 事件錯誤，修改為檢查鼠標 <code>y</code> 軸 &lt; 1 時判斷移動是否在瀏覽器標籤頁上。</p>
<hr />
...
• 長按鼠標模式下按下鍵盤不觸發預覽窗口。
• 拖拽模式下拖拽鏈接到新標籤時不會觸發預覽窗口。</p>
<hr />
<h3><strong>📅 2024/8/29 08:39 - Ver 2.4.0.9</strong></h3>
<p><strong>新增</strong>: 拖拽時增加超時顯示進度條，時間耗盡時取消拖拽打開預覽窗口。</p>
<hr />
<h3><strong>📅 2024/8/29 06:28 - Ver 2.4.0.8</strong></h3>
<p><strong>最佳化</strong>: 移除長按時監聽鼠標移動的邏輯，改為監聽鼠標進入拖拽狀態時取消打開預覽窗口。<a href="https://greasyfork.org/zh-CN/scripts/504880-small-window-preview/discussions/257270#comment-526172">#526172</a></p>
<hr />
<h3><strong>📅 2024/8/27 00:33 - Ver 2.4.0.5</strong></h3>
<p><strong>修復</strong>: <a href="https://greasyfork.org/zh-CN/scripts/504880-small-window-preview/discussions/257270#comment-526054">#526054</a> 反饋的 BUG：
• 修復在關閉預覽窗後無法觸發時間條打開預覽窗的概率性問題。</p>
<hr />
...
• 添加長按時顯示倒計時，增加記住窗口位置功能。</p>
<hr />
<h3><strong>📅 2024/8/24 07:29 - Ver 2.4</strong></h3>
<p><strong>最佳化</strong>: 添加語言包，優化腳本性能。</p>
<hr />
<h3><strong>📅 2024/8/23 08:12 - Ver 2.4</strong></h3>
<p><strong>最佳化</strong>: 兼容 Chrome 插件 <code>Google Chrome 的超級拖拽 0.9.9</code>。
<strong>修復</strong>: 修復點擊小窗口關閉按鈕時亞克力圖層不關閉的問題。</p></details>
...
🔗 拖拽即開小窗 — 拖拽網頁鏈接，便捷地在小窗口中打開並預覽，使用 Edge 的預讀技術。小窗口後方有亞克力效果，提升視覺體驗。
...
1. **操作模式 (actionMode)**
    ...
    - **描述**：設置當前的操作模式，根據 `actionMode` 的值來確定使用哪個操作模式。
        - **操作模式 1**：`1` - 在鏈接長按時觸發預覽窗口。
        - **操作模式 2**：`2` - 在鏈接拖拽時觸發預覽窗口。
        - **默認操作模式**：`0` - 默認啟用以上兩種操作模式。

2. **長按持續時間 (longPressDuration)**

    - **描述**：設置長按操作的持續時間。
    - **操作**：`setLongPressDuration`，用於修改長按持續時間，與 `操作模式 (actionMode)` 配合使用。

3. **模糊效果啟用 (blurEnabled)**

    - **描述**：啟用或禁用背景模糊效果。
    - **操作**：`toggleBlurEffect`，用於啟用或禁用模糊效果。

4. **模糊強度 (blurIntensity)**

    - **描述**：設置背景模糊的強度。

5. **點擊鼠標時關閉 (closeOnMouseClick)**

    - **描述**：在主窗口（預覽窗口的父窗口）點擊鼠標時關閉預覽窗口。
    - **操作**：`toggleCloseOnMouseClick`，用於啟用或禁用點擊鼠標時關閉預覽窗口的設置。

6. **滾動時關閉 (closeOnScroll)**

    - **描述**：在主窗口（預覽窗口的父窗口）滾動鼠標滑輪時關閉預覽窗口。
    - **操作**：`toggleCloseOnScroll`，用於啟用或禁用滾動鼠標時關閉預覽窗口的設置。

7. **窗口寬度 (windowWidth)**

    - **描述**：設置預覽窗口的寬度，啟用 `保存窗口配置` 時覆蓋該配置。

8. **窗口高度 (windowHeight)**

    - **描述**：設置預覽窗口的高度，啟用 `保存窗口配置` 時覆蓋該配置。

9. **顯示倒計時 (showCountdown)**

    - **描述**：當啟用 `操作模式 (actionMode)=1` 長按時，在長按觸發預覽窗口前，在鼠標下方顯示倒計時進度條，時間是 `長按持續時間 (longPressDuration)` 設定的時間。
    - **操作**：用於啟用或禁用倒計時進度條顯示。

10. **顯示拖拽超時 (showCountdowndrag)**

-   **描述**：當啟用時，在拖拽預覽窗口前，在鼠標下方顯示超時進度條，時間是 `拖拽超時時間 (dragTimeOut)` 設定的時間,時間耗盡,拖拽不會打開預覽窗口,設定時間為0時為禁用顯示以及禁用超時取消拖拽打開小窗預覽。
-   **操作**：用於啟用或禁用超時進度條顯示。

11. **保存窗口配置 (saveWindowConfig)**

    -   **描述**：用於記憶保存窗口的大小與位置配置，會覆蓋用戶自己設定的預覽窗口高度和寬度。

**每個選項後面都有一個對應的操作函數，這些函數用於更新菜單的設置。通過點擊選項，用戶可以直接改變相關配置，並立即看到效果。**

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 支持的網頁

🌐 全網頁適用 — 支持所有網頁，隨時隨地享受便捷的鏈接預覽與打開功能。
...
點擊或滾動頁面時，可以自動關閉小窗口。
立即體驗！
🚀 提升瀏覽效率 — 安裝腳本，享受智能鏈接預覽和美觀小窗口！

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## ☭ 感謝

-   **[hiisme](https://greasyfork.org/zh-CN/users/217852) [小窗預覽](https://greasyfork.org/scripts/504567)** 是本腳本的創始人,他的想法與實際行動為互聯網廣大網友做出了巨大貢獻
-   **[wxhzjm](https://greasyfork.org/zh-CN/users/1118488)**
-   **[hmjz100](https://greasyfork.org/zh-CN/users/893941)**
-   **[tony0809](https://greasyfork.org/zh-CN/users/20361)** 為腳本添加了自定義域名設置的代碼
