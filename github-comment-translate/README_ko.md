...
<center><div align="center">
    ...
    <img src="https://img.shields.io/greasyfork/dt/530312?logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&label=%EC%82%AC%EC%9A%A9%EC%9E%90%20%EC%88%98">
    <img src="https://img.shields.io/badge/dynamic/json?&label=%EB%A7%A4%EC%9D%BC%20%ED%99%95%EC%9D%B8%20%EC%97%85%EB%8D%B0%EC%9D%B4%ED%8A%B8&query=$.530312&logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&url=https://github.com/ChinaGodMan/UserScriptsHistory/raw/main/daily_update_checks.json">
...
이 스크립트는 키보드 바로 가기에 들어갑니다 `Ctrl + Q`，선택한 텍스트를 번역하고 번역을 대체하는 기능 구현 원본 텍스트 상자。번역이 호출되었습니다 Google Translate API。

## 기능 설명

1. **바로 가기 키 트리거**：누르다 `Ctrl + Q` 번역 함수가 트리거 될 때。
2. **텍스트 번역을 선택하십시오**：텍스트 상자에서 선택한 텍스트를 자동으로 감지합니다，대상 언어로 번역하십시오。
3. **번역 결과 교체**：번역 된 텍스트를 원본 텍스트 상자에서 선택한 위치로 바꾸십시오.。
4. 지원하다 GitHub ~에`issues`그리고`pull`페이지`markdown`정보를 번역하십시오。
5. 스크립트에서 필요한 번역 언어를 사용자 정의 할 수 있습니다.,기본적으로 영어로 번역됩니다。`const translate = 'en'`

## 스크립트 로직

1. 키보드 이벤트를 듣습니다，압축되었는지 여부를 결정하십시오 `Ctrl + Q`。
2. 대상 텍스트 상자에서 선택한 텍스트가 감지 된 경우：
    - 선택된 텍스트를 추출하십시오。
    - 부르다 Google Translate API 번역하십시오。
    - 원래 선택한 텍스트를 번역 결과로 바꾸십시오。
3. 텍스트가 선택되지 않았거나 대상 텍스트 상자를 찾을 수없는 경우，신속한 정보는 콘솔에서 출력됩니다。
