...
<center><div align="center">
    ...
    <img src="https://img.shields.io/greasyfork/dt/507036?logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&label=%E7%94%A8%E6%88%B6%E6%95%B8%E9%87%8F">
    <img src="https://img.shields.io/badge/dynamic/json?&label=%E6%AF%8F%E6%97%A5%E6%AA%A2%E6%9F%A5%E6%9B%B4%E6%96%B0%E6%AC%A1%E6%95%B8&query=$.507036&logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&url=https://github.com/ChinaGodMan/UserScriptsHistory/raw/main/daily_update_checks.json">
    <p>Download:<a href="https://github.com/ChinaGodMan/UserScripts/tree/main/colorful-table">Github</a> | ⭐<a
            ...
            Fork</a></p><details><summary style="color:#FFD700">更新記錄</summary><div align="right"><a href="https://github.com/ChinaGodMan/UserScripts/tree/main/colorful-table/CHANGELOG.md"><img src="https://img.shields.io/badge/-GitHub-3A3A3A?style=flat&logo=GitHub&logoColor=white"></a></div><h1><strong>🛠️ 絢麗漸變美化網頁表格 更新日誌</strong></h1>
<h3><strong>📅 2024/9/6 05:41 - Ver 1.0.0.0</strong></h3>
<p><strong>修改</strong>: 修改為自動漸變色.....</p>
<hr /></details>
...
該腳本會自動為網頁中的所有表格應用顏色漸變，無需用戶手動點擊或操作，基於表格列中的數值大小生成不同的背景色，從而增強表格的視覺效果。

#### 功能特點

1. **自動應用漸變色**：腳本會自動掃描頁面中的表格，為每一列中的數據應用漸變色，無需用戶手動觸發。
2. **數據驅動的漸變色**：根據每一列中數值的最小值和最大值，生成顏色漸變效果，數值越大或越小，背景顏色變化越明顯。
3. **持續檢測**：腳本每秒檢查一次頁面，如果有新表格加載，自動為其應用顏色漸變。
4. **漸變色配置**：漸變顏色基於HSL色彩模型，腳本中可調整色調範圍（`HUE_RANGE`）、飽和度（`SATURATION`）、亮度（`LIGHTNESS`）等參數。

#### 主要參數

-   **HUE_RANGE**: 設置漸變色的色調範圍，默認值為120程度，表示綠色到紅色的漸變。
-   **SATURATION**: 控制顏色的飽和度，默認為80%，提供了明亮的顏色效果。
-   **LIGHTNESS**: 控制顏色的亮度，默認值為88%，使漸變色柔和且不刺眼。

#### 核心邏輯

1. **extractFirstFloat**: 從單元格文本中提取第一個浮點數，忽略非數字內容，確保只使用數值部分生成顏色漸變。
2. **applyGradient**: 為每個表格列自動計算最大和最小值，然後根據單元列的數值，生成相應的漸變背景色。
3. **initializeTables**: 定期檢查頁面中的表格，並自動為未初始化的表格應用漸變色。

#### 使用場景

該腳本適用於任何包含網頁表格場景，能夠自動為網頁表格應用顏色漸變，幫助用戶快速識別數據的差異，提升表格的可視化效果。
...
-   **原始腳本：** [yearly](https://greasyfork.org/zh-CN/users/1312063)創建的[彩色表格 | 表格數據可視化](https://greasyfork.org/zh-CN/scripts/502933)
