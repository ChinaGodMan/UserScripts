...
一個 Tampermonkey 腳本，用於在 GreasyFork 上增強討論列表功能。當你的腳本或你參與的討論有新回复時，腳本會在網頁上以模態窗口顯示最新的討論內容。
...
腳本使用 `delay` 配置項來設定檢查更新的間隔時間。時間格式為以下幾種組合：
...
你可以根據需要修改 `delay` 配置項的值，以調整腳本檢查討論更新的頻率。例如，如果你希望每15分鐘檢查一次，可以將 `delay` 設置為 `15m`。
