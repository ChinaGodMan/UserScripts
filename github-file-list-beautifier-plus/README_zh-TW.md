...
<center><div align="center">
    ...
    <p>「 GitHub 文件清單美化器是一個使用者腳本，用於增強 GitHub 倉庫中的文件顯示效果。它可以為文件和資料夾添加顏色，並將文件類型圖示替換為小圖像，使得程式碼庫更加易於瀏覽和管理。按類型為文件添加顏色和圖標，在儲存庫來源樹中顯示小圖像以取代文件類型圖標 」</p>
    ...
    <img src="https://img.shields.io/greasyfork/dt/508047?logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&label=%E4%BD%BF%E7%94%A8%E8%80%85%E6%95%B8%E9%87%8F">
    <img src="https://img.shields.io/badge/dynamic/json?&label=%E6%AF%8F%E6%97%A5%E6%AA%A2%E6%9F%A5%E6%9B%B4%E6%96%B0%E6%AC%A1%E6%95%B8&query=$.508047&logo=greasyfork&logoColor=white&labelColor=%23670000&color=blue&url=https://github.com/ChinaGodMan/UserScriptsHistory/raw/main/daily_update_checks.json">
...
GitHub 文件列表美化器是一個用戶腳本，用於增強 GitHub 倉庫中的文件顯示效果。它可以為文件和文件夾添加顏色，並將文件類型圖標替換為小圖像，使得代碼庫更加易於瀏覽和管理。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 功能

-   **文件夾圖標**：將文件夾圖標替換為更直觀的文件夾圖標。
-   **自定義圖標**：根據文件的擴展名或文件名，顯示自定義的圖標。
-   **圖片文件預覽**：對於圖片文件，自動顯示縮略圖。
-   **顏色標記**：為不同類型的文件添加顏色，以便更快速地識別文件類型。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 請注意:

**本腳本目就是為了著色不同的文件類型,顯示圖標只是附帶的,有需要 Github 顯示圖標的,建議下載**
...
1. **安裝 Tampermonkey 或 Greasemonkey 外掛**：

    - [Tampermonkey](https://www.tampermonkey.net/)（支援 Chrome, Firefox, Edge, Opera 等瀏覽器）
    - [Greasemonkey](https://www.greasespot.net/)（支援 Firefox）

2. **添加腳本**：

    - 點擊 安裝到 Tampermonkey Greasemonkey 。

3. **腳本配置**：
    - 腳本首次運行時會從指定 URL 加載顏色配置。如果無法加載配置，將使用默認設置。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 使用

-   腳本會在 GitHub 頁面加載時自動運行，處理文件列表中的文件圖標。
-   可以根據需要在 `localStorage` 或腳本設置中調整圖標和顏色配置。
...
-   絕對名稱：(用來顯示圖標的,暫時沒添加絕對文件的自定義著色)
    `
"123.txt": {
    "color": "red",
    "icon": "javascript"
},`
-   默認從 vscode-material-icon-theme 獲取圖標：
    `    "123.txt": {
    "color": "red",
    "icon": "javascript"
},`
...
-   **瀏覽器**：支援 Chrome、Firefox、Edge、Opera 和 Safari。
...
-   該腳本基於 [MIT 許可證](https://opensource.org/licenses/MIT) 開源。

<img height=6px width="100%" src="https://media.chatgptautorefresh.com/images/separators/gradient-aqua.png?latest">

## 貢獻

-   本腳本由 [wOxxOm](https://greasyfork.org/zh-CN/users/2159) [GitHub file list beautifier](https://greasyfork.org/zh-CN/scripts/5982) 和 人民的勤务员 開發。
-   更多信息請訪問 [GitHub 項目主頁](https://github.com/ChinaGodMan/UserScripts)。
