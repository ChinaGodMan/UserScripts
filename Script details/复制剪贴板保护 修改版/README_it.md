...
## Introduzione dello script

Lo script scaricato altrove，L'autore non sa chi sia molto bravo，Prevenire la psoriasi。utilizzo ChatGPT Modificata，Fare la sceneggiatura PC 端浏览时可以使utilizzo快捷键复制，Copia varie password in background della pagina web speciale。Lo script e le restrizioni di copia potrebbero non essere compatibili！

Quando lo script è efficace，Visualizzerà un piccolo punto traslucido nell'angolo in basso a destra della pagina web，Colori diversi corrispondono a stati diversi，descritto come segue：

- **rosso** : Intercettare tutto il comportamento di copia，E la notifica pop -up è autorizzata。
- **verde** : Lascia che tutti copia tutta la copia，È uguale allo script non viene eseguito。
- **colore arancione** : Sollevamento e intercettazione temporanei，Questo stato appare solo quando copia manuale，Diventa automaticamente rosso dopo che l'utente ha completato la copia，Riprendi l'intercettazione。
- **Il dot rosso non viene visualizzato**，Verrà visualizzato dopo aver rilevato la scrittura degli appunti 4s。È possibile fare clic sullo stato dell'interruttore in questo momento，Scegli se mettere su queste pagine web per scrivere il comportamento。
