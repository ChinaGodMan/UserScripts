...
### Chinese (Simplified) / 중국인（쉽게 한）
이 사용자 스크립트는 모든 웹사이트의 링크 밑줄을 제거합니다.，새 링크를 동적으로 처리합니다.。더욱 깨끗하고 시각적으로 매력적인 브라우징 경험을 보장합니다.。

### Chinese (Traditional) / 중국인（중국어 번체）
이 사용자 스크립트는 모든 사이트에서 링크 자막을 제거합니다.，새 링크를 동적으로 처리합니다.。더욱 깨끗하고 시각적으로 매력적인 브라우징 경험을 보장합니다.。
...
### Japanese / 일본어
このユーザースクリプトは、임의의のWebサイト우수한のすべてのリンクの오프라인を삭제し、새로운しいリンクを움직일 수 있는に처리します。これにより、よりクリーンで시각적に매력なブラウジング체험が보증されます。
...
## Author
국민의 하인 (Servant of the People) & ChatGPT
